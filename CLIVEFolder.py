...
class CLIVEFolder(data.Dataset):
...
if __name__ == '__main__':
    TIDroot = '/DATA/gaoyixuan_data/tid2013'
    index = list(range(0,3000))
    random.shuffle(index)
    train_index = index[0:round(0.8*3000)]
    test_index = index[round(0.8*3000):3000]
    trainset = TIDFolder(root = TIDroot, loader = default_loader, index = train_index)
    testset = TIDFolder(root = TIDroot, loader = default_loader, index = test_index)
