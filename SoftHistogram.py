...
class SoftHistogram(nn.Module):
    ...
    def __init__(self,n_features,n_examples,num_bins,quantiles=False):
        ...
        self.bin_widths_conv.bias.data.fill_(1)#bias固定
        self.bin_widths_conv.bias.requires_grad = False#不学习bias
        
        self.hist_pool = nn.AvgPool1d(n_examples,stride=1,padding=1) #average by row
    ...
    def forward(self,input):
        ...
        # print(input.unsqueeze(0).unsqueeze(0).permute(0,2,1).size())
        # print(input.size())
        input = self.bin_centers_conv(input.unsqueeze(0).unsqueeze(0).permute(0,2,1))
